...
class Indexcm extends models{
    //订餐系统后台更新页面逻辑处理
     public function updatePage()
     {
        ...
        //模板路径
        $templateFile=APP_PPDP."/home/template_shop.html";
        ...
        preg_match('#admin#',$adminName['data'],$res);

        if(!empty($adminName['data']))
        {
         ...
         if(isset($_SESSION['shopId'])){
             ...
             $shopId=$shopId['data'];
             //生成文件
             $shopFile=APP_PATH."/views/home/shopc/shop/".$shopId.".html";
         }
        ...
        if($row){
            $shopInfoTxt="";
            $shopInfoTemplate="<img class='shop-icon' src='{APP_UPLOAD}#shopIcon#' >"
                ."<span class='shop-name' shopId='#shopId#' shopName='#shopName#' shopPhone='17600361387'>"
                ."#shopName#"
                ."</span>"
                ."<span class='switch-address'>"
                ."<a class='switch-address' href='?m=home&c=Indexc&a=index'>[切换地址]</a>"
                ."</span>";


            $v1=str_replace("#shopIcon#",$row[0]['shopIcon'],$shopInfoTemplate);
            $v2=str_replace("#shopId#",$row[0]['shopId'],$v1);
            $v3=str_replace("#shopName#",$row[0]['shopName'],$v2);
            $shopInfoTxt=$v3."\n"; //基本信息

            $shopTipTxt=$row[0]['shopTip'];//公告
        ...
        for($i=0;$i<count($cates);$i++){
            ...
            if($rows){
                $menuItemTemplate="<div class='menu-item' item-id='#id#'>"
                    ."<div class='item-wrap'>"
                    ."<img src='{APP_UPLOAD}upload/goods_image_thumb_220/#icon#' height='130' width='130'> "
                    ."<div class='item-detail'>"
                    ."<span class='name '>#name#</span>"
                    ."<span class='price'  item-price='#price#'>￥#price#</span> "
                    ."<img class='buy' src='{IMAGE}home/icon_buy.png' > "
                    ."<ul class='stars'>"
